...
from .layers.transformer_head import TransformerDecoder, LinearPts3d, ClassificationHead
...
class Pi3(nn.Module, PyTorchModelHubMixin):
    ...
    def forward(self, imgs):
        ...
        hidden_for_classification = torch.cat([intermediate_outputs[2], intermediate_outputs[3], intermediate_outputs[-2], intermediate_outputs[-1]], dim=-1)
        classification_hidden = self.classification_decoder(hidden_for_classification, xpos=pos)
        ...
        with torch.amp.autocast(device_type='cuda', enabled=False):
            ...
            # classification
            classification_hidden = classification_hidden.float()
            logits, feat = self.classification_head(classification_hidden, N, self.patch_start_idx)
